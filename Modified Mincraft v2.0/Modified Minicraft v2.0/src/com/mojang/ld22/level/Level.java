...
import com.mojang.ld22.entity.Archer;
...
public class Level {
	...
	public int depth, timeLim;
	public int monsterDensity = random.nextInt(8)+4;
	...
	public Level(int w, int h, int level, Level parentLevel) {
		timeLim = random.nextInt(500)+500;
		if (level < 0) {
			dirtColor = 222;
		}
		this.depth = level;
		...
		for (int i = 0; i < count; i++) {
			...
			if (random.nextInt(2) == 0)
				...
				if (random.nextInt(100) <= 80)
					...
					mob = new Archer(lvl);//new Archer(lvl);
	...
	public void trySpawn(int count, int Type) {
		for (int i = 0; i < count; i++) {
			Mob mob;
