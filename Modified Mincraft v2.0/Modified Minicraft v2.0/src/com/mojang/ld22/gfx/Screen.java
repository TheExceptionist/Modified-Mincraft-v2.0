	...
	public void render(int xp, int yp, int tile, int colors, int bits) {
		...
		for (int y = 0; y < 8; y++) {
			...
			for (int x = 0; x < 8; x++) {
				...
				int xs = x;
				if (mirrorX) xs = 7 - x;
				//System.out.println((sheet.pixels[xs + ys * sheet.width + toffs] * 8));
				//0 - 8 - 16 - 24 - assuming rbg pixels of sheet
	//			System.out.println(colors >> (sheet.pixels[xs + ys * sheet.width + toffs] * 8));
