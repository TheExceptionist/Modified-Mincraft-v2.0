...
public class HardRockTile extends Tile {
	...
	public void render(Screen screen, Level level, int x, int y) {
		...
		//System.out.println(this+" "+r+" "+l+" "+d+" "+u);
		...
		if (item instanceof ToolItem) {
			...
			if (tool.type == ToolType.pickaxe && tool.level == 4) {
				if (player.payStamina(4 - tool.level)) {
					hurt(level, xt, yt, random.nextInt(10) + (tool.level) * 5 + 10 + ToolItem.attributes_bonus[tool.attr][random.nextInt(ToolItem.attributes_bonus[tool.attr].length)]);
					return true;
				}
			}
		}
